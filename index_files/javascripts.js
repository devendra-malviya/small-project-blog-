...
if(i_m.value=='NaN' || !i_m.value) { return false; }
var number = parseInt(i_m.value) + randomnumber;
var new_value = format_numbers(number);
write_new_value(new_value,'div_i_m');
i_m.value = number;
var i = document.getElementById('i');
if(i.value=='NaN' || !i.value) { return false; }
var number = parseInt(i.value) + randomnumber;
var new_value = format_numbers(number);
write_new_value(new_value,'div_i');
i.value = number;
}
function add_clicks() {
...
var c_m = document.getElementById('c_m');
if(c_m.value=='NaN' || !c_m.value) { return false; }
var number = parseInt(c_m.value) + randomnumber;
var new_value = format_numbers(number);
write_new_value(new_value,'div_c_m');
c_m.value = number;
var c = document.getElementById('c');
if(c.value=='NaN' || !c.value) { return false; }
var number = parseInt(c.value) + randomnumber;
var new_value = format_numbers(number);
write_new_value(new_value,'div_c');
c.value = number;
}
...
{ for(i=0; i<form_id.elements.length; i++)
  { if (form_id.elements[i].type=='checkbox')
    ...
    { request += form_id.elements[i].name+"="+form_id.elements[i].value+'&';
      //alert("The field name is: " + form_id.elements[i].name + " and its value is: " + form_id.elements[i].value);
    }
  }
}
//alert(request);
...
{ for(i=0; i<form_element.elements.length; i++)
  { if (form_element.elements[i].type=='checkbox')
    { //alert("The field name is: " + form_element.elements[i].name+ " and its value is: " + form_element.elements[i].checked);
	  if (form_element.elements[i].checked==true) request += form_element.elements[i].name+'=1&';
    }
    else if (form_element.elements[i].type=='radio')
    { //alert("RADIO FIELD: " + form_element.elements[i].name+ " VALUE: " + form_element.elements[i].value);
    ...
    { request += form_element.elements[i].name+"="+form_element.elements[i].value+'&';
      //alert("The field name is: " + form_element.elements[i].name + " and its value is: " + form_element.elements[i].value);
    }
  }
}
if (window.XMLHttpRequest) { http_request = new XMLHttpRequest(); } 
...
function show_ajax_result(http_request,element_id,redirect_url) {
	//alert(http_request.readyState+'--'+http_request.status+'--'+element_id);
  //alert(redirect_url);
